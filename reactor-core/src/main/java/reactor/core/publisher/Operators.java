...
public abstract class Operators {
    ...
    public static Throwable onOperatorError(@Nullable Subscription subscription,
                                            ...
                                            Context context // 本次处理过程中的上下文对象
    ) {

        // 判断该异常是否需要强制抛出
        Exceptions.throwIfFatal(error);
        // 当出现异常时关闭 subscription
        ...
        // 从当前上下文中尝试获取 当触发 error时使用的 钩子函数
     ...
     * @param field    the field accessor    subscriber 内部有关 subscription的原子更器
     * @param instance the parent instance  代表 subscriber
     * @param s        the subscription to set once   将要订阅的数据
     * @return true if successful, false if the target was not empty or has been cancelled
     * 为 subscriber 指定 subscription 这样 订阅者就可以消费内部的元素了
     */
    public static <F> boolean setOnce(AtomicReferenceFieldUpdater<F, Subscription> field, F instance, Subscription s) {
        ...
        Subscription a = field.get(instance);
        // 如果发现 subscription 对象已经被关闭了 触发 cancel 并返回false
        if (a == CancelledSubscription.INSTANCE) {
            s.cancel();
            return false;
        }
        // 如果 subscriber 已经设置了 subscription 打印日志
        if (a != null) {
            s.cancel();
        ...
        // 设置  subscriber 内部的 subscription
        ...
        // 再次判断设置的 subscription 是否是  cancelledSubscription 实例
        if (a == CancelledSubscription.INSTANCE) {
            s.cancel();
            return false;
        }

        // 代表发生并发 竞争失败 只能关闭本次尝试使用的 subscription  同时打印日志
        s.cancel();
        reportSubscriptionSet();
        return false;
    }
    ...
    public static <F> boolean terminate(AtomicReferenceFieldUpdater<F, Subscription> field,
                                        F instance) {
        // 获取该对象内部 设置的 subscriber
        Subscription a = field.get(instance);
        if (a != CancelledSubscription.INSTANCE) {
            // 修改成CancelledSubscription
    ...
    /**
     * 简言之 下限小的 按照下限发送  下限大于推荐值 使用 3/4 prefetch
     * @param prefetch
     * @param lowTide
     * @return
     */
    ...
    /**
     * 该对象会在被设置到 subscriber 中时 触发 cancel
     */
    ...
    public static class MonoSubscriber<I, O>
         ...
         *          代表某个数据被处理完成了 由该对象的子类来决定该方法的触发时机
         */
        ...
        public void request(long n) {
            if (validate(n)) {
                for (; ; ) {
                    ...
                    // 代表已经触发过一次request了 忽略 request 请求
                    if ((s & ~NO_REQUEST_HAS_VALUE) != 0) {
                        return;
                    }
                    // 如果此时已经生成了 value 将数据传递到下游
     ...
     *           代表 多个 subscription 与 一个 subscriber 的 适配对象  每个 subsciption 对应一个 pub
     */
    abstract static class MultiSubscriptionSubscriber<I, O>
        ...
        /**
         * 当request 传入的参数是 MaxValue 时 该标识设置成true代表不再需要调用request 方法了
         */
        ...
        /**
         * 该对象也看作一个 subscriber  为该对象设置一个上游对象  并触发上游的request 方法
         * @param s
         */
        @Override
        public void onSubscribe(Subscription s) {
        ...
        public final void request(long n) {
            if (validate(n)) {
                ...
                if (wip == 0 && WIP.compareAndSet(this, 0, 1)) {
                    ...
                    // 获取当前的 subscription 上游有多个 pub 每个都会对应一个 subscription 对象
                    ...
                    // 因为该对象只是一个适配对象  实际上 request 还是要发送到上游真正的 subscripton 中
                    if (a != null) {
                        a.request(n);
                    }
        ...
        public final void set(Subscription s) {
            ...
            if (wip == 0 && WIP.compareAndSet(this, 0, 1)) {
                ...
                // 更新当前subscription 并关闭旧的  默认情况是不关闭
        ...
        final void drainLoop() {
            ...
            for (; ; ) {
                ...
                // 最终触发此刻指向的 subscription.request
