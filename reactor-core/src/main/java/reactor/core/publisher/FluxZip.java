                    ...
                    for (int j = 0; j < n; j++) {
                        ...
                        if (values[j] == null) {
                            try {
                                ...
                                // 只要某个 pub 的数据发送完了就会关闭整个 subscriber
                                if (d && sourceEmpty) {
                                    cancelAll();
                                ...
                                if (!sourceEmpty) {
                                    ...
                                    // 使用情况 应该是 多个 pub 会发射相同数量的元素 每当pub 都准备好一个元素后
                                    // empty 才会为 false 这样会将 同批次的数据 压缩成一个数据 并触发 onNext
                                    empty = true;
                                }
                            } catch (Throwable ex) {
                        ...
                        if (values[j] == null) {
                            try {
                                ...
                                if (d && empty) {
                                    ...
                                    // 只要有其中一个pub 数据发送完了 就会停止整个 subscriber
                                    a.onComplete();
                                    return;
                                }
                                if (!empty) {
